...
namespace Farmacia_Arqui_Soft.Application.Services.UserServices
{
    public class UserService : IUserService
    {
        ...
        public async Task<User> RegisterAsync(UserCreateDto dto, int actorId)
        {
            ...
            var pre = _validator.Validate(user);
            if (!pre.IsValid) throw new ValidationException(pre.Errors);

            // Manteniendo tu GetAll() por orden del profe
            var existing = await _repo.GetAll();

            // Username
            var baseUsername = _username.BuildBase(user.first_name, user.second_name, user.last_name, user.ci);
            user.username = _username.EnsureUnique(baseUsername, existing.Select(x => x.username));
            ...
            // Password
            var plainPassword = _pwdGen.Generate(12);
            user.password = _hasher.Hash(plainPassword);
        ...
        public async Task UpdateAsync(int id, UserUpdateDto dto, int actorId)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dto.Password))
                current.password = _hasher.Hash(dto.Password!);
