...
namespace Farmacia_Arqui_Soft.Domain.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<User> RegisterAsync(UserCreateDto dto, int actorId)
        {
            ...
            // 1) Construir entidad con nombres/rol/etc.
            var user = new User
            {
                first_name = dto.FirstName.Trim(),
                second_name = string.IsNullOrWhiteSpace(dto.SecondName) ? null : dto.SecondName!.Trim(),
                last_name = dto.LastName.Trim(),
                username = "",                 // se setea más abajo
                password = "",                 // se setea más abajo (hash)
                mail = dto.Mail.Trim(),
                phone = dto.Phone,
                ci = dto.Ci.Trim(),
                role = dto.Role,
            ...
            // 4) Generar password aleatoria, hashearla y guardar
        ...
        public async Task<User?> GetByIdAsync(int id)
            => await _repo.GetById(new User { id = id }); // compat con tu firma actual
        ...
        public async Task UpdateAsync(int id, UserUpdateDto dto, int actorId)
        {
            ...
            if (dto.SecondName is not null) current.second_name = string.IsNullOrWhiteSpace(dto.SecondName) ? null : dto.SecondName.Trim();
            if (dto.LastName is not null) current.last_name = dto.LastName.Trim();
        ...
        {
            // Hasta que el repo exponga GetByUsername, usamos GetAll()
        ...
        // ---------- Username ----------
        public static string GenerateUsernameFromNames(string first, string? second, string last)
        {
            static string Initial(string? s) => string.IsNullOrWhiteSpace(s) ? "" : s.Trim()[0].ToString();

            var raw = (Initial(first) + Initial(second) + last).ToLowerInvariant();
            // quitar tildes
        ...
        // ---------- Password (PBKDF2 en un solo campo) ----------
        ...
        {
            if (length < 8) length = 8; // mínimo decente
        ...
        symbols[RandomNumberGenerator.GetInt32(symbols.Length)]
            ...
            // mezclar de forma criptográficamente aleatoria
